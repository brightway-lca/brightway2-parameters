...
def test_call_updates_exchanges():
    ds = {
        ...
        "exchanges": [{"formula": "Elders_of_Krikkit"}, {"amount": 44}],
    }
    ParameterSet(ds["parameters"])(ds["exchanges"])
    assert ds["parameters"]["East_River_Creature"] == {
        "amount": 100,
...
def test_simple_evaluation():
    params = {
        ...
        "Gargravarr": {
            "formula": (
                "Agrajag + Constant_Mown + Deep_Thought + "
...
def test_missing_fields():
    ...
    with pytest.raises(ValueError):
        ParameterSet(
            [{"amount": 1, "name": "Gag_Halfrunt"}, {"name": "Emily_Saunders"}]
        )
    with pytest.raises(ValueError):
        ParameterSet(
            [{"formula": "foo", "name": "Deep_Thought"}, {"name": "Constant_Mown"}]
        )
